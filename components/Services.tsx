...
const Services: React.FC = () => {
  ...
  const services = [
    ...
    {
      title: "UI/UX Design",
      desc: "Swiss-inspired user interfaces with a focus on typography, grid systems, and functional clarity."
    }
  ];

  return (
    <div ref={ref} className={`py-20 transition-opacity duration-1000 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
      <div className="mb-16">
        <h2 className="text-xs font-bold uppercase tracking-[0.2em] text-swiss-red mb-2">What I Do</h2>
        <h3 className="text-4xl md:text-6xl font-black text-m3-on-surface tracking-tight">CAPABILITIES</h3>
      </div>
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 md:gap-8 items-start">
              <div className="md:col-span-1">
                 <span className="text-xs font-mono text-m3-secondary opacity-50 group-hover:opacity-100 group-hover:text-swiss-red transition-colors">0{index + 1}</span>
              </div>
